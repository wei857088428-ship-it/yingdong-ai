...
type PolishedShot = { shot_number: number; dialogue: string; sound: string; duration_seconds: number; speaker_name: string; speaker_voice: SpeakerVoiceProfile; continuity_state: string };
...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const schema = { type: "object", additionalProperties: false, required: ["shots"], properties: { shots: { type: "array", minItems: shots.length, maxItems: shots.length, items: { type: "object", additionalProperties: false, required: ["shot_number","dialogue","sound","duration_seconds","speaker_name","speaker_voice","continuity_state"], properties: { shot_number: { type: "integer" }, dialogue: { type: "string" }, sound: { type: "string" }, duration_seconds: { type: "integer", minimum: 2, maximum: 15 }, speaker_name: { type: "string" }, speaker_voice: { type: "string", enum: ["female","male","neutral"] }, continuity_state: { type: "string", minLength: 20, description: "Exact end-of-shot location, time, character positions/facing, clothing, injuries, held props, lighting, and final pose; inherit every unchanged fact from the prior shot." } } } } } };
    ...
    const response = await fetch("https://api.x.ai/v1/chat/completions", { method: "POST", headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" }, cache: "no-store", body: JSON.stringify({ model: "grok-4.5", temperature: 0.25, max_tokens: 8000, response_format: { type: "json_schema", json_schema: { name: "polished_dialogue", strict: true, schema } }, messages: [
      { role: "system", content: "每个镜头都必须重建 continuity_state 状态账本：写清镜头结束时的地点、时间、所有人物站位与朝向、服装、伤势、手持道具、光线和最终姿势。上一镜没有被可见事件改变的事实必须逐项原样继承，不能重置或遗漏。" },
    ...
    const stateShots = shots.map((shot) => ({ ...shot, continuity_state: String(polished.find((item) => item.shot_number === shot.shot_number)?.continuity_state ?? "").trim() }));
    ...
    for (const item of polished) {
      ...
      const currentState = stateShots[shotIndex]; const state = currentState.continuity_state;
      const cleanImagePrompt = String(current.image_prompt ?? "").replace(/\n?\[CONTINUITY STATE\]\s*\n?[^\n]*/gi, "");
      const cleanVideoPrompt = String(current.video_prompt ?? "").replace(/\n?\[CONTINUITY STATE\]\s*\n?[^\n]*/gi, "");
      const imagePrompt = withContinuityPrompt(`${cleanImagePrompt}\n[CONTINUITY STATE]\n${state}`, currentState, stateShots[shotIndex - 1], stateShots[shotIndex + 1], "image");
      const videoPrompt = withContinuityPrompt(`${cleanVideoPrompt}\n[CONTINUITY STATE]\n${state}`, currentState, stateShots[shotIndex - 1], stateShots[shotIndex + 1], "video");
