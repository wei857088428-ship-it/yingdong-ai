...
type PolishedShot = { shot_number: number; dialogue: string; sound: string; duration_seconds: number; speaker_name: string; speaker_voice: SpeakerVoiceProfile; causal_link: string; continuity_state: string };
...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const schema = { type: "object", additionalProperties: false, required: ["shots"], properties: { shots: { type: "array", minItems: shots.length, maxItems: shots.length, items: { type: "object", additionalProperties: false, required: ["shot_number","dialogue","sound","duration_seconds","speaker_name","speaker_voice","causal_link","continuity_state"], properties: { shot_number: { type: "integer" }, dialogue: { type: "string" }, sound: { type: "string" }, duration_seconds: { type: "integer", minimum: 2, maximum: 15 }, speaker_name: { type: "string" }, speaker_voice: { type: "string", enum: ["female","male","neutral"] }, causal_link: { type: "string", minLength: 10, description: "Concrete visible prior cause and why it directly triggers this shot; for shot 1, the opening trigger and immediate objective." }, continuity_state: { type: "string", minLength: 20, description: "Exact end-of-shot location, time, character positions/facing, clothing, injuries, held props, lighting, and final pose; inherit every unchanged fact from the prior shot." } } } } } };
    ...
    const response = await fetch("https://api.x.ai/v1/chat/completions", { method: "POST", headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" }, cache: "no-store", body: JSON.stringify({ model: "grok-4.5", temperature: 0.25, max_tokens: 12000, response_format: { type: "json_schema", json_schema: { name: "polished_dialogue", strict: true, schema } }, messages: [
      { role: "system", content: "每个镜头都必须同时重建 causal_link 和 continuity_state。causal_link 具体写出上一镜可见事件为何直接触发本镜动作，第1镜写开场触发事件与人物当下目标，禁止只写‘剧情继续’。continuity_state 写清镜头结束时的地点、时间、所有人物站位与朝向、服装、伤势、手持道具、光线和最终姿势。上一镜没有被可见事件改变的事实必须逐项原样继承，不能重置或遗漏。" },
    ...
    const stateShots = shots.map((shot) => { const repaired=polished.find((item) => item.shot_number === shot.shot_number);return { ...shot, causal_link: String(repaired?.causal_link ?? "").trim(), continuity_state: String(repaired?.continuity_state ?? "").trim() }; });
    ...
    for (const item of polished) {
      ...
      const currentState = stateShots[shotIndex]; const causalLink = currentState.causal_link; const state = currentState.continuity_state;
      const removeContracts = (prompt: string) => prompt.replace(/\n?\[CAUSAL LINK\]\s*\n?[^\n]*/gi, "").replace(/\n?\[CONTINUITY STATE\]\s*\n?[^\n]*/gi, "");
