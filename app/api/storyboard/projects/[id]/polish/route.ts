...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const schema = { type: "object", additionalProperties: false, required: ["shots"], properties: { shots: { type: "array", minItems: shots.length, maxItems: shots.length, items: { type: "object", additionalProperties: false, required: ["shot_number","dialogue","sound","duration_seconds"], properties: { shot_number: { type: "integer" }, dialogue: { type: "string" }, sound: { type: "string" }, duration_seconds: { type: "integer", minimum: 2, maximum: 15 } } } } } };
    ...
    const response = await fetch("https://api.x.ai/v1/chat/completions", { method: "POST", headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" }, cache: "no-store", body: JSON.stringify({ model: "grok-4.5", temperature: 0.25, max_tokens: 8000, response_format: { type: "json_schema", json_schema: { name: "polished_dialogue", strict: true, schema } }, messages: [
      { role: "system", content: `你是竖屏漫剧对白导演。逐镜修复对白和表演指令，但不能改变事件、人物关系、地点、道具和结局。要求：每镜只允许一个人说话；台词口语化、有明确目的和潜台词；不能复述画面；每秒最多约4个汉字；相邻镜头情绪变化必须由事件触发。dialogue 无需说话时可为空。sound 必须以“表演：”开头，写清主情绪、强度1-5、语速、音量、停顿和潜台词，再写环境声与动作音效。duration_seconds 必须足够说完台词并留0.5秒呼吸。保持 shot_number 不变。` },
