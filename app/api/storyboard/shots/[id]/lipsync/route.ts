...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (["completed", "complete", "done", "success"].includes(status) && videoUrl) {
    ...
    try { durableVideoUrl = await archiveGeneratedVideo(videoUrl, user.id, "lipsync", `${id}-${jobId}`); }
    catch (error) { console.error("Archive HeyGen lip-sync video failed; using provider URL:", error); }
    const updates: Record<string, unknown> = { video_url: durableVideoUrl, media_status: "lipsync_ready", error_message: null };
