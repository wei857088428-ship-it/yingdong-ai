...
export default function Dashboard() {
  ...
  return <main className="ai-shell">
    ...
    <section className="studio">
      <header className="studio-head"><div><b>影动漫剧 1.3</b><span>{panel === "workflow" ? "从创意到成片" : panel === "works" ? "作品库" : `${mode === "chat" ? "编剧" : mode === "image" ? "画面" : "镜头"}模式`}</span>{panel === "create" && mode !== "chat" && <select className="character-select" value={selectedCharacterId} onChange={(event) => setSelectedCharacterId(event.target.value)}><option value="">不绑定角色</option>{characters.map((character) => <option value={character.id} key={character.id}>{character.name} V{character.version}</option>)}</select>}</div><Link href="/">返回首页</Link></header>
