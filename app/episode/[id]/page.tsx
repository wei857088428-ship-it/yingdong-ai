...
export default function EpisodePage() {
  ...
  async function exportMp4(){ if(!project||exporting)return; const missing=shots.filter((item)=>!item.video_url&&!item.image_url); if(missing.length)return setExportStatus(`还有 ${missing.length} 个镜头没有画面，请先完成整集生成`); setExporting(true);setExportProgress(0);setExportStatus("正在加载本地视频渲染器…");
    try { const [{FFmpeg}]=await Promise.all([import("@ffmpeg/ffmpeg")]); const ffmpeg=new FFmpeg(); ffmpeg.on("progress",({progress})=>setExportProgress(Math.max(1,Math.min(99,Math.round(progress*100))))); await ffmpeg.load({coreURL:"/ffmpeg/ffmpeg-core.js",wasmURL:"/ffmpeg/ffmpeg-core.wasm"}); const {fetchFile}=await import("@ffmpeg/util"); const segmentNames:string[]=[];
