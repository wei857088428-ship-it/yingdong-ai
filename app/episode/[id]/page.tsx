...
export default function EpisodePage() {
  ...
  async function exportMp4(){ if(!project||exporting)return; const missing=shots.filter((item)=>!item.video_url&&!item.image_url); if(missing.length)return setExportStatus(`还有 ${missing.length} 个镜头没有画面，请先完成整集生成`); setExporting(true);setExportProgress(0);setExportStatus("正在加载本地视频渲染器…");
    try { const [{FFmpeg},{fetchFile,toBlobURL}]=await Promise.all([import("@ffmpeg/ffmpeg"),import("@ffmpeg/util")]); const ffmpeg=new FFmpeg(); ffmpeg.on("progress",({progress})=>setExportProgress(Math.max(1,Math.min(99,Math.round(progress*100))))); const core="https://cdn.jsdelivr.net/npm/@ffmpeg/core@0.12.10/dist/umd"; await ffmpeg.load({coreURL:await toBlobURL(`${core}/ffmpeg-core.js`,"text/javascript"),wasmURL:await toBlobURL(`${core}/ffmpeg-core.wasm`,"application/wasm")}); const segmentNames:string[]=[];
      for(let i=0;i<shots.length;i++){const item=shots[i];const duration=String(Math.max(2,item.duration_seconds));setExportStatus(`正在渲染镜头 ${i+1}/${shots.length}…`);setExportProgress(Math.round(i/shots.length*90));const visual=item.video_url||item.image_url!;const sourceName=`visual-${i}.${item.video_url?"mp4":"img"}`;const audioName=`audio-${i}.mp3`;const subtitleName=`subtitle-${i}.png`;const segmentName=`segment-${i}.mp4`;await ffmpeg.writeFile(sourceName,await fetchFile(visual));if(item.audio_url)await ffmpeg.writeFile(audioName,await fetchFile(item.audio_url));await ffmpeg.writeFile(subtitleName,await fetchFile(await subtitleOverlay(item.dialogue||"")));const sourceArgs=item.video_url?["-i",sourceName]:["-loop","1","-framerate","30","-t",duration,"-i",sourceName];const audioArgs=item.audio_url?["-i",audioName]:["-f","lavfi","-t",duration,"-i","anullsrc=channel_layout=stereo:sample_rate=44100"];await ffmpeg.exec([...sourceArgs,...audioArgs,"-loop","1","-i",subtitleName,"-filter_complex",`[0:v]scale=720:1280:force_original_aspect_ratio=decrease,pad=720:1280:(ow-iw)/2:(oh-ih)/2:black,setsar=1,tpad=stop_mode=clone:stop_duration=${duration}[base];[base][2:v]overlay=0:0:format=auto[v]`,"-map","[v]","-map","1:a","-af","apad","-t",duration,"-r","30","-c:v","libx264","-preset","ultrafast","-crf","28","-pix_fmt","yuv420p","-c:a","aac","-ar","44100","-ac","2","-movflags","+faststart",segmentName]);segmentNames.push(segmentName);}
