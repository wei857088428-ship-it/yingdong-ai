...
export function performanceSignature(value?: string) {
  ...
  const categoryGroups: Array<[string, RegExp]> = [
    ...
    ["tender", /温柔|关切|安慰|心疼|深情/], ["determined", /坚定|果断|决绝|命令|保护|守护/],
    ["suspicious", /怀疑|试探|警惕|疑惑/], ["nervous", /紧张|焦虑|不安|慌张/],
    ["restrained", /克制|压抑|隐忍|冷静|冰冷|麻木/], ["shocked", /震惊|惊讶|错愕/],
  ...
  const pace = /语速[^；。]*(?:快|急)|快速|急促/.test(direction) ? "fast" : /语速[^；。]*(?:慢|缓|迟)|缓慢|慢慢/.test(direction) ? "slow" : "normal";
