...
import { hasCrossEpisodeLedger } from "./seriesContinuityRepair";
...
export function auditSeriesQuality(projects:AuditProject[],characters:AuditCharacter[]):SeriesQualityReport {
  ...
  projects.forEach((project,episodeIndex)=>{
    ...
    if(episodeIndex>0&&!hasCrossEpisodeLedger(projects[episodeIndex-1],project)){const first=project.storyboard_shots.toSorted((left,right)=>left.shot_number-right.shot_number)[0];add({level:"warning",code:"missing_cross_episode_ledger",message:`第 ${episodeNumber} 集首镜没有锁定上一集结尾的服装、伤势、道具、站位与光线状态，建议执行旧剧集连续性修复`,projectId:project.id,episodeNumber,shotNumber:first?.shot_number});}
