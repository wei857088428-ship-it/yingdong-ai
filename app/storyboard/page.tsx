...
import { planSeriesBindings } from "@/app/lib/seriesBindingPlan";
import { projectsNeedingContinuityRepair } from "@/app/lib/seriesContinuityRepair";
...
export default function StoryboardPage() {
  ...
  async function pollGeneratedVideo(requestId:string,shotNumber:number,shouldStop:()=>boolean=()=>false){
    if(!requestId)throw new Error("视频任务已创建但没有返回任务编号，请联系管理员核对任务记录");
    let consecutiveFailures=0;
    for(let attempt=0;attempt<120&&!shouldStop();attempt++){
      await new Promise((resolve)=>setTimeout(resolve,5000));let check:Response;let detail:{status?:string;videoUrl?:string;error?:string};
      try{check=await fetch(`/api/status?requestId=${encodeURIComponent(requestId)}`,{cache:"no-store"});detail=await check.json();}
      catch{consecutiveFailures++;if(videoPollDecision(0,"",consecutiveFailures)==="retry"){setStatus(`镜头 ${shotNumber} 的视频任务仍在运行，网络短暂中断，正在自动重连 ${consecutiveFailures}/6…`);continue;}throw new Error("连续多次无法查询视频任务，生成请求没有重复提交；稍后可恢复未完成任务");}
      const decision=videoPollDecision(check.status,detail.status??"",check.ok?0:++consecutiveFailures);
      if(decision==="retry"){setStatus(`镜头 ${shotNumber} 的视频任务仍在运行，服务暂时繁忙，正在重试 ${consecutiveFailures}/6…`);continue;}
      if(decision==="error")throw new Error(detail.error||`查询视频任务失败（${check.status}）`);
      consecutiveFailures=0;if(decision==="failed")throw new Error(detail.error||"视频生成失败，积分已退还");
      if(decision==="completed"){if(!detail.videoUrl)throw new Error("视频任务已完成但没有返回文件地址");return detail.videoUrl;}
    }
    return "";
  }
    ...
    try {
      ...
      for (let index = 0; index < working.length; index++) {
        ...
        try {
          ...
          await updateShot(shot.id, { status: "video_generating", error: "" }); const response = await fetch("/api/generate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ provider: "xai", prompt: shot.video_prompt, image: shot.image_url, duration: Math.min(15, shot.duration_seconds), aspectRatio: "9:16", resolution: videoResolution, characterIds: effectiveCharacterIds(shot), batch: true }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error); const videoUrl=await pollGeneratedVideo(String(data.requestId||""),shot.shot_number); if (!videoUrl) throw new Error("视频仍在处理中，请稍后重试"); shot = await updateShot(shot.id, { videoUrl, status: "completed", error: "" }); working[index] = shot;
        }
  ...
  async function repairSeriesBindings(){
    if(batching)return;const plan=planSeriesBindings(activeSeries,characters);if(!plan.changes.length)return setStatus(plan.missingCharacters.length||plan.unresolvedSpeakers?`没有可自动修复的绑定；仍缺少 ${plan.missingCharacters.length} 个角色档案、${plan.unresolvedSpeakers} 个无法判断说话人的镜头，请先人工补齐`:`全季角色档案、说话人和固定音色已经一致`);
    const invalidationNote=plan.visualInvalidations||plan.audioInvalidations?`\n\n为避免新旧角色混用，将清除 ${plan.visualInvalidations} 个受影响镜头的旧画面，以及 ${plan.audioInvalidations} 段错误音色配音/口型，之后可由制作队列重新生成。`:"";
    if(!window.confirm(`将自动修复 ${plan.changes.length} 个镜头：${plan.castRepairs} 处角色档案绑定、${plan.speakerRepairs} 处说话人、${plan.voiceRepairs} 处固定音色。${plan.missingCharacters.length?`仍有 ${plan.missingCharacters.length} 个未创建角色无法自动绑定。`:""}${plan.unresolvedSpeakers?`仍有 ${plan.unresolvedSpeakers} 个多角色对白镜头需要人工指定说话人。`:""}${invalidationNote}\n\n确定修复吗？`))return;
    setBatching(true);let completed=0;try{for(const change of plan.changes){setStatus(`正在修复全季角色与声音绑定 ${completed+1}/${plan.changes.length} · 镜头 ${change.shotNumber}`);const body:Record<string,string|string[]|null>={};if(change.characterIds)body.characterIds=change.characterIds;if(change.speakerCharacterId)body.speakerCharacterId=change.speakerCharacterId;await patchProjectShot(change.projectId,change.shotId,body);completed++;}setSeriesAudit(null);setStatus(`全季绑定修复完成：更新 ${completed} 个镜头；${plan.visualInvalidations} 个旧画面、${plan.audioInvalidations} 段旧配音已按需失效，可启动全季制作队列重新生成${plan.missingCharacters.length||plan.unresolvedSpeakers?`。仍需人工处理 ${plan.missingCharacters.length} 个角色档案和 ${plan.unresolvedSpeakers} 个说话人`:""}`);}catch(error){setStatus(`已修复 ${completed} 个镜头，后续停止：${error instanceof Error?error.message:"修复失败"}`);}finally{setBatching(false);}
  }

  async function repairLegacySeriesContinuity(){
    if(batching||!legacyContinuityProjects.length)return;const generatedShots=legacyContinuityProjects.flatMap((project)=>project.storyboard_shots).filter((shot)=>shot.image_url||shot.video_url).length;
    if(!window.confirm(`将用 AI 重新修复 ${legacyContinuityProjects.length} 集旧续集的剧情因果、首镜服装、伤势、道具、站位和光线连续性，预计消耗 ${legacyContinuityProjects.length} 积分。最多 ${generatedShots} 个已有画面可能因提示词变化而失效，系统会避免新旧状态混用。确定继续吗？`))return;
    setBatching(true);let completed=0;let invalidatedVisuals=0;try{for(const project of legacyContinuityProjects){setStatus(`正在修复旧剧集跨集连续性 ${completed+1}/${legacyContinuityProjects.length} · ${project.title}`);const response=await fetch(`/api/storyboard/projects/${project.id}/polish`,{method:"POST"});const data=await response.json();if(!response.ok)throw new Error(data.error);const updatedShots=data.shots as Shot[];invalidatedVisuals+=Number(data.invalidatedVisuals||0);setProjects((current)=>current.map((item)=>item.id===project.id?{...item,storyboard_shots:updatedShots}:item));if(project.id===currentProjectId)setShots(updatedShots);completed++;}setSeriesAudit(null);setStatus(`旧剧集连续性修复完成：已重建 ${completed} 集首镜状态契约，${invalidatedVisuals} 个不再匹配的旧画面已失效，可启动全季制作队列重新生成`);}catch(error){setSeriesAudit(null);setStatus(`已成功修复 ${completed} 集，后续停止：${error instanceof Error?error.message:"连续性修复失败"}。已完成内容会保留，可再次点击继续。`);}finally{setBatching(false);}
  }
  ...
  const legacyContinuityProjects=projectsNeedingContinuityRepair(activeSeries);
